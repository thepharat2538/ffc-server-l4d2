...
public Plugin myinfo =
{
	...
	author = "DingbatFlat, HarryPotter, BatoSaiX",
...
public Action Timer_PlayerLeftCheck(Handle Timer)
{
	if (LeftStartArea())
	{
		if (!LeftSafeRoom) {
			LeftSafeRoom = true;
			SelectImprovedTarget();
			//("[sb_fix] Survivors left the safe area.");
		}
...
{
	// ("type %i, leftsaferoom %b", c_iSelectType, LeftSafeRoom);
	...
	if (c_iSelectType == 0) {
		...
		//("\x04Type1 - %d bot%s", c_iSelectNumber, (c_iSelectNumber == 1) ? "" : "s");
		...
		for (int x = 1; x <= MaxClients; x++) {
			if (isSurvivorBot(x)) {
				g_bFixTarget[x] = true;
				count++;
				//("\x04(%d/%d)\x05. %N", count, c_iSelectNumber, x);
