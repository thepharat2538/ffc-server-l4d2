...
public Plugin myinfo = 
{
	...
	author		= "Buster \"Mr. Zero\" Nielsen, HarryPotter, PS_FatCat",
...
{
	LoadTranslations("l4d_vocalize_antiflood.phrases");
	
	hConVar1 = CreateConVar("l4d_vocalize_antiflood_player_token_time", "5", "ผู้เล่นต้องใช้เวลากี่วินาทีในการปลดบล็อคการใช้บทพูด", FCVAR_NOTIFY, true, 1.0);
	hConVar2 = CreateConVar("l4d_vocalize_antiflood_world_token_time", "2", "เวิลด์ต้องใช้เวลากี่วินาทีในการปลดบล็อคจากการใช้บทพูด", FCVAR_NOTIFY, true, 1.0);
	hConVar3 = CreateConVar("l4d_vocalize_antiflood_player_token_limit", "5", "จำนวนครั้งที่ผู้เล่นจะใช้บทพูด (-1 = ไม่จำกัด)", FCVAR_NOTIFY, true, -1.0);
	hConVar4 = CreateConVar("l4d_vocalize_antiflood_world_token_limit", "5", "จำนวนครั้งที่เวิลด์จะใช้บทพูด (-1 = ไม่จำกัด)", FCVAR_NOTIFY, true, -1.0);
	hConVar5 = CreateConVar("l4d_vocalize_antiflood_notify", "1", "ถ้าตั้งเลข 1, จะแจ้งเตือนให้ผู้เล่นที่ถูกบล็อค", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hImmueAccess = CreateConVar("l4d_vocalize_antiflood_immue_flag", "-1", "สิทธิ์ผู้เล่นที่จะได้รับข้อยกเว้นจากการบล็อคนี้ (ปล่อยว่าง = ทุกคน, -1 = ไม่มีใคร)", FCVAR_NOTIFY);
...
bool IsPlayerVocalizeFlooding(int client, int initiator)
{
	...
	else
	{
		...
		{
			if(g_bMessage) PrintToChat(client, "[FFC] %t", "Vocalize_Notify", client);
			return true;
		}
